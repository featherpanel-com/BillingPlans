...
class PlansController
{
    ...
    public function get(Request $request, int $planId): Response
    {
        ...
        $preloaded = [
            "activeCounts" => [
                (int) $plan["id"] => Plan::getActiveSubscriptionCount(
                    (int) $plan["id"],
                ),
            ],
            "allRealms" => Realm::getAll(null, 500, 0) ?: [],
    ...
    public function subscribe(Request $request, int $planId): Response
    {
        ...
        $skipInitialCharge = (bool) $request->attributes->get(
            "_billingplans_skip_initial_charge",
            false,
        );
        $forcedInitialPeriodDaysRaw = $request->attributes->get(
            "_billingplans_initial_period_days",
        );
        $forcedInitialPeriodDays =
            $forcedInitialPeriodDaysRaw !== null
                ? max(1, (int) $forcedInitialPeriodDaysRaw)
                : null;
        $subscriptionSource = (string) $request->attributes->get(
            "_billingplans_subscription_source",
            "manual",
        );
        ...
        $customResources =
            isset($input["custom_resources"]) &&
            is_array($input["custom_resources"])
                ? $input["custom_resources"]
                : [];
            ...
            foreach ($sliderConfig as $resourceKey => $config) {
                if (!empty($config["enabled"])) {
                    ...
                    $selectedVal = isset($customResources[$resourceKey])
                        ...
                        : $baseVal;
                    $selectedVal = max($baseVal, min($maxVal, $selectedVal));
                    ...
                    $finalVal = $baseVal + $steps * $stepVal;
                    $finalVal = max($baseVal, min($maxVal, $finalVal));

                    $cleanCustomResources[$resourceKey] = $finalVal;
                    $plan[$resourceKey] = $finalVal;
                }
            }
        }
        if (!empty($cleanCustomResources)) {
            $plan["custom_resources"] = $cleanCustomResources;
        }

        $chargeBreakdown = Plan::calculateChargeBreakdown($plan);
        $totalChargeCredits = (int) $chargeBreakdown["total_credits"];
        $chargeCreditsNow = $totalChargeCredits;
        $periodDays = max(1, (int) ($plan["billing_period_days"] ?? 30));
        $couponCode = strtoupper(trim((string) ($input["coupon_code"] ?? "")));
        $couponContext = null;

        if (!$skipInitialCharge && $couponCode !== "") {
            $couponResult = $this->resolveCouponForSubscription(
                $userId,
                (int) $planId,
                $couponCode,
                $totalChargeCredits,
            ...
            if (!$couponResult["ok"]) {
                return ApiResponse::error(
                    ...
                    $couponResult["error_code"] ?? "COUPON_INVALID",
                    400,
                );
            }
            $couponContext = $couponResult["context"] ?? null;
            if (is_array($couponContext)) {
                $chargeCreditsNow = max(
                    0,
                    (int) ($couponContext["initial_charge_credits"] ??
                        $totalChargeCredits),
                );
            }
        }
        ...
        if ($maxSubscriptions !== null && $maxSubscriptions > 0) {
            ...
            if ($activeCount >= $maxSubscriptions) {
                return ApiResponse::error(
                    "This plan is sold out right now.",
                    "PLAN_SOLD_OUT",
                    400,
                );
            }
        ...
        if (!$skipInitialCharge) {
            ...
            if ($currentCredits < $chargeCreditsNow) {
                return ApiResponse::error(
                    "Insufficient credits for this plan.",
        ...
        if ($effectiveSpellId && $effectiveRealmId) {
            ...
            $serverResult = $this->provisionServer(
                $planForProvision,
                $user,
            ...
            if ($serverResult["success"]) {
                ...
                $rawProvisionError =
                    (string) ($serverResult["error"] ?? "Unknown error");
                $publicError =
                    "Failed to provision server. Please contact staff.";

                if (
                    $provisionCode === "WINGS_ERROR" ||
                    str_contains(strtolower($rawProvisionError), "cURL error 7")
                ) {
                    $publicError =
                        "Target node is offline. Please contact staff team.";
                } elseif (
                    $provisionCode === "NO_AVAILABLE_NODE" ||
                    $provisionCode === "NO_FREE_ALLOCATIONS"
                ) {
                    $publicError =
                        "No suitable node is currently available. Please contact staff team.";
                }

                return ApiResponse::error($publicError, $provisionCode, 500);
            }
        }

        $initialPeriodDays = $forcedInitialPeriodDays ?? $periodDays;
        $nextRenewal = date(
            "Y-m-d H:i:s",
            strtotime("+{$initialPeriodDays} days"),
        );
        $subscriptionId = Subscription::create([
            "user_id" => $userId,
            "plan_id" => $planId,
            "coupon_code_id" => is_array($couponContext)
                ? (int) ($couponContext["code_id"] ?? 0)
                : null,
            "coupon_code" => is_array($couponContext)
                ? $couponContext["code"] ?? null
                : null,
            "coupon_scope" => is_array($couponContext)
                ? $couponContext["scope"] ?? null
                : null,
            "renewal_discount_percent" => is_array($couponContext)
                ? $couponContext["renewal_discount_percent"] ?? null
                : null,
            "renewal_discount_credits" => is_array($couponContext)
            ...
            "custom_resources" => !empty($cleanCustomResources)
                ? json_encode($cleanCustomResources)
                : null,
        ]);
        ...
        if (is_array($couponContext) && !empty($couponContext["code_id"])) {
            $this->markCouponUsed($userId, (int) $couponContext["code_id"]);
        }
        ...
        Activity::createActivity([
            ...
            "context" =>
                "User subscribed to plan: {$plan["name"]} (ID: {$planId}). Subscription #$subscriptionId. Paid: " .
                ($skipInitialCharge ? "0 (trial)" : $chargeCreditsNow) .
                " credits. Source: {$subscriptionSource}." .
        ...
        return ApiResponse::success(
            [
                ...
                "credits_deducted" => $skipInitialCharge
                    ? 0
                    : $chargeCreditsNow,
                "credits_before_discount" => $skipInitialCharge
                    ? 0
                    : $totalChargeCredits,
                "base_credits" => (int) $chargeBreakdown["base_credits"],
                "tax_credits" => (int) $chargeBreakdown["tax_credits"],
                "extra_charge_credits" =>
                    (int) $chargeBreakdown["extra_charge_credits"],
            ...
            $skipInitialCharge
                ? "Successfully activated trial plan!"
    ...
    private function provisionServer(
        ...
        try {
            ...
            if (!$spell) {
                return [
                    "success" => false,
                    "error" => "Spell not found or was deleted from the panel",
            ...
            if ($realm === null) {
                return [
                    "success" => false,
                    "error" => "Realm not found or was deleted from the panel",
            ...
            if ((int) ($spell["realm_id"] ?? 0) !== $realmId) {
                return [
                    "success" => false,
                    "error" =>
                        "Spell does not belong to the selected realm; fix the plan template in admin.",
            ...
            if (!$nodeId) {
                return [
                    "success" => false,
                    "error" =>
                        "No selected node has enough free resources and a free allocation.",
            ...
            $startup = !empty($plan["startup_override"])
                ? $plan["startup_override"]
                : $spell["startup"] ?? "";
            $image = !empty($plan["image_override"])
                ? $plan["image_override"]
                : $spell["docker_image"] ?? "";
            if (empty($image) && !empty($spell["docker_images"])) {
                $images = is_string($spell["docker_images"])
                    ? json_decode($spell["docker_images"], true)
                    : $spell["docker_images"];
                if (is_array($images)) {
            ...
            if ($image === "") {
                return [
                    "success" => false,
                    "error" =>
                        "Spell has no Docker image configured. Set docker image on the spell or an image override on the plan.",
            ...
            $uuidShort = substr(
                str_replace("-", "", $this->generateUuidV4()),
                0,
                8,
            );
            ...
            $serverData = [
                ...
                "skip_scripts" => 0,
                "oom_disabled" => 0,
                "allocation_limit" => !empty($plan["allocation_limit"])
                    ? (int) $plan["allocation_limit"]
                    : null,
            ...
            if (!$serverId) {
                return [
                    "success" => false,
                    "error" =>
                        "Could not save the server in the database. Check panel logs.",
            ...
            if (!Allocation::assignToServer($allocationId, $serverId)) {
                ...
                return [
                    "success" => false,
                    "error" =>
                        "Could not assign a network allocation to the server.",
            ...
            if (!empty($variablesToCreate)) {
                ServerVariable::createOrUpdateServerVariables(
                    $serverId,
                    $variablesToCreate,
                );
            }
            ...
            $app->getLogger()->info(
                "BillingPlans: Server {$uuid} provisioned for user #{$userId} (plan: {$plan["name"]}).",
            );

            return ["success" => true, "uuid" => $uuid];
        } catch (\Throwable $e) {
            $app->getLogger()->error(
                "BillingPlans: Server provisioning failed: " . $e->getMessage(),
            );
    ...
    /**
     * Try each node in order, return first enabled node with a free allocation.
     *
     * This intentionally mirrors admin manual server creation behavior, which
     * does not hard-block on node usage counters before attempting creation.
     *
     * @param int[] $nodeIds
     * @param array{memory:int,disk:int,cpu:int} $requirements
     */
    private function resolveProvisionNode(
        array $nodeIds,
        array $requirements,
    ): ?int {
        foreach ($nodeIds as $nodeId) {
            if ($this->nodeCanProvision($nodeId)) {
                return $nodeId;
            }
        }
    ...
    /**
     * Check if node exists, is enabled, and has at least one free allocation.
     */
    private function nodeCanProvision(int $nodeId): bool
    {
        $node = Node::getNodeById($nodeId);
        if (!$node) {
            return false;
     ...
     */
    private function getFreeNodeAllocations(
        int $nodeId,
        int $limit = 100,
    ): array {
        $allocs =
            Allocation::getAll(null, $nodeId, null, $limit, 0, true) ?: [];
        ...
        $fallback =
            Allocation::getAll(null, $nodeId, null, $limit, 0, false) ?: [];
        if (empty($fallback)) {
            return [];
        }

        return array_values(
            array_filter($fallback, static function (array $alloc): bool {
    ...
    private function resolvePlanLocations(array $plan): array
    {
        ...
        return [
            "ids" => array_values(
                array_map("intval", array_keys($locationIds)),
            ),
            "names" => $locationNames,
        ];
    }
     ...
     */
    private function resolveRealmOptions(
        array $plan,
     ...
     */
    private function resolveSpellOptions(
        array $plan,
        array $allowedSpellIds,
        array $allowedRealmOptions,
        array &$preloaded,
    ): array {
        $options = [];
        $allowedRealmSet = [];
            ...
            if (!empty($allowedSpellIds)) {
                foreach ($allowedSpellIds as $spellId) {
                    ...
                    if ($spell !== null) {
                        ...
                        if (
                            !empty($allowedRealmSet) &&
                            !isset($allowedRealmSet[$realmId])
                        ) {
                            continue;
                        }
                        $options[] = [
    ...
    private function getRealmByIdCached(int $realmId, array &$preloaded): ?array
    {
        ...
        if (!array_key_exists($realmId, $preloaded["realmById"])) {
            ...
            $preloaded["realmById"][$realmId] =
                $match ?: Realm::getById($realmId);
        }
    ...
    private function getSpellByIdCached(int $spellId, array &$preloaded): ?array
    {
        ...
        if (!array_key_exists($spellId, $preloaded["spellById"])) {
            ...
            $preloaded["spellById"][$spellId] =
                $match ?: Spell::getSpellById($spellId);
        }
    ...
    /**
     * @return array{ok:bool,message?:string,error_code?:string,context?:array<string,mixed>}
     */
    private function resolveCouponForSubscription(
        int $userId,
        int $planId,
        string $couponCode,
        int $totalChargeCredits,
    ): array {
        if (!class_exists(RedeemCode::class)) {
            return [
                "ok" => false,
                "message" => "Coupon system is not available.",
                "error_code" => "COUPON_UNAVAILABLE",
            ];
        }
        $code = RedeemCode::getByCode($couponCode);
        if (!$code || !RedeemCode::isValid($code)) {
            return [
                "ok" => false,
        ...
        if (($code["reward_type"] ?? "credits") !== "billing_plan_coupon") {
            return [
                "ok" => false,
                "message" => "This code cannot be used as a checkout coupon.",
                "error_code" => "COUPON_WRONG_TYPE",
            ];
        }
        $targetPlanId = isset($code["plan_id"]) ? (int) $code["plan_id"] : 0;
        if ($targetPlanId > 0 && $targetPlanId !== $planId) {
            return [
                "ok" => false,
                "message" => "This coupon is for a different plan.",
                "error_code" => "COUPON_PLAN_MISMATCH",
            ];
        }
        if (
            class_exists(RedeemUsage::class) &&
            RedeemUsage::hasUserUsedCode($userId, (int) $code["id"])
        ) {
            return [
                "ok" => false,
        ...
        $scope = (string) ($code["coupon_scope"] ?? "initial");
        if (!in_array($scope, ["initial", "renewal", "both"], true)) {
            $scope = "initial";
        }
        $discountPercent = max(
            0.0,
            min(100.0, (float) ($code["discount_percent"] ?? 0)),
        );
        $discountCredits = max(0, (int) ($code["discount_credits"] ?? 0));
        if ($discountPercent <= 0 && $discountCredits <= 0) {
            return [
                "ok" => false,
                "message" => "Coupon discount configuration is invalid.",
                "error_code" => "COUPON_MISCONFIGURED",
            ];
        }

        $percentDiscountCredits = (int) round(
            $totalChargeCredits * ($discountPercent / 100),
            0,
            PHP_ROUND_HALF_UP,
        );
        $initialDiscountCredits = in_array($scope, ["initial", "both"], true)
            ? min(
                $totalChargeCredits,
                max(0, $percentDiscountCredits + $discountCredits),
            )
            : 0;
        $initialChargeCredits = max(
            0,
            $totalChargeCredits - $initialDiscountCredits,
        );
        ...
        return [
            ...
            "context" => [
                ...
                "initial_discount_credits" => $initialDiscountCredits,
                "initial_charge_credits" => $initialChargeCredits,
                "renewal_discount_percent" => in_array(
                    $scope,
                    ["renewal", "both"],
                    true,
                )
                    ...
                    : null,
                "renewal_discount_credits" => in_array(
                    $scope,
                    ["renewal", "both"],
                    true,
                )
    ...
    private function markCouponUsed(int $userId, int $codeId): void
    {
        try {
            if (
                !class_exists(RedeemUsage::class) ||
                !class_exists(RedeemCode::class)
            ) {
                return;
            }
            if (RedeemUsage::hasUserUsedCode($userId, $codeId)) {
                return;
            }
            if (
                RedeemUsage::recordUsage($userId, $codeId) &&
                RedeemCode::incrementUses($codeId)
            ) {
                return;
            }
            App::getInstance(false, true)
                ->getLogger()
                ->warning(
                    "BillingPlans: coupon usage tracking failed for code #$codeId user #$userId.",
                );
        } catch (\Throwable $e) {
            App::getInstance(false, true)
                ->getLogger()
                ->error(
                    "BillingPlans: Failed to mark coupon used: " .
                        $e->getMessage(),
                );
        }
    }
