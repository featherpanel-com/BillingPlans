...
class SubscriptionsController
{
    ...
    public function list(Request $request): Response
    {
        ...
        foreach ($subscriptions as &$sub) {
            ...
            $sub['allowed_upgrade_plan_ids'] = Plan::decodeIds($sub['allowed_upgrade_plan_ids'] ?? null);
            $sub['allowed_downgrade_plan_ids'] = Plan::decodeIds($sub['allowed_downgrade_plan_ids'] ?? null);
        }
    ...
    public function get(Request $request, int $subscriptionId): Response
    {
        ...
        $subscription['allowed_upgrade_plan_ids'] = Plan::decodeIds($subscription['allowed_upgrade_plan_ids'] ?? null);
        $subscription['allowed_downgrade_plan_ids'] = Plan::decodeIds($subscription['allowed_downgrade_plan_ids'] ?? null);
    ...
    public function changePlan(Request $request, int $subscriptionId): Response
    {
        ...
        if (!in_array((string) $subscription['status'], ['active', 'suspended'], true)) {
            return ApiResponse::error('Only active or suspended subscriptions can be changed', 'INVALID_SUBSCRIPTION_STATUS', 400);
        }
        ...
        if ($newPlanId <= 0) {
            return ApiResponse::error('Invalid plan ID', 'INVALID_PLAN_ID', 400);
        }
        $currentPlanId = (int) ($subscription['plan_id'] ?? 0);
        if ($newPlanId === $currentPlanId) {
            return ApiResponse::error('Subscription is already on this plan', 'PLAN_UNCHANGED', 400);
        }

        $newPlan = Plan::getById($newPlanId);
        if ($newPlan === null || (int) ($newPlan['is_active'] ?? 0) !== 1) {
            return ApiResponse::error('Target plan not found or inactive', 'PLAN_NOT_FOUND', 404);
        }

        $oldPlan = Plan::getById($currentPlanId);
        if ($oldPlan === null) {
            return ApiResponse::error('Current plan not found', 'CURRENT_PLAN_NOT_FOUND', 404);
        }

        $maxSubscriptions = isset($newPlan['max_subscriptions']) && $newPlan['max_subscriptions'] !== null
        ...
        if ($maxSubscriptions !== null && $maxSubscriptions > 0) {
            ...
            if ($activeCount >= $maxSubscriptions) {
                return ApiResponse::error('The selected plan is sold out right now.', 'PLAN_SOLD_OUT', 400);
            }
        }
        ...
        $allowedUpgradeIds = Plan::decodeIds($oldPlan['allowed_upgrade_plan_ids'] ?? null);
        $allowedDowngradeIds = Plan::decodeIds($oldPlan['allowed_downgrade_plan_ids'] ?? null);
        $hasExplicitRules = !empty($allowedUpgradeIds) || !empty($allowedDowngradeIds);
        if ($hasExplicitRules) {
            if ($delta > 0 && !in_array($newPlanId, $allowedUpgradeIds, true)) {
                return ApiResponse::error('This plan cannot be upgraded to the selected target.', 'UPGRADE_NOT_ALLOWED', 403);
            }
            if ($delta < 0 && !in_array($newPlanId, $allowedDowngradeIds, true)) {
                return ApiResponse::error('This plan cannot be downgraded to the selected target.', 'DOWNGRADE_NOT_ALLOWED', 403);
            }
            if ($delta === 0 && !in_array($newPlanId, array_values(array_unique(array_merge($allowedUpgradeIds, $allowedDowngradeIds))), true)) {
                return ApiResponse::error('Switching to the selected plan is not allowed.', 'PLAN_SWITCH_NOT_ALLOWED', 403);
            }
        }
        ...
        if ($delta > 0) {
            ...
            if ($credits < $delta) {
                return ApiResponse::error("Insufficient credits. You need {$delta} credits to upgrade.", 'INSUFFICIENT_CREDITS', 400);
            }
            ...
            if (!CreditsHelper::addUserCredits($userId, $refund)) {
                return ApiResponse::error('Failed to refund credits.', 'CREDITS_REFUND_FAILED', 500);
            }
        }

        $nextRenewal = date('Y-m-d H:i:s', strtotime('+' . max(1, (int) ($newPlan['billing_period_days'] ?? 30)) . ' days'));
        if (!Subscription::update($subscriptionId, [
            ...
            return ApiResponse::error('Failed to change subscription plan', 'PLAN_CHANGE_FAILED', 500);
        }
        ...
        return ApiResponse::success([
            ...
            'next_renewal_at' => $nextRenewal,
        ], 'Subscription plan changed successfully', 200);
    }
}
